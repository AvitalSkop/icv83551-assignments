...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        q = q.view(N, S, H, D).transpose(1, 2)
        v = v.view(N, T, H, D).transpose(1, 2)
        
        # (3) dot product scaled attention scores
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(D)
        
        # (4) apply attention mask if there is one
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, input_dim, num_heads, dim_feedforward=2048, dropout=0.1):
        ...
        self.res_drop1 = nn.Dropout(dropout)
        self.res_drop2 = nn.Dropout(dropout)
        self.res_drop3 = nn.Dropout(dropout)
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        tmp = self.self_attn(query=tgt, key=tgt, value=tgt, attn_mask=tgt_mask)
        tgt = tgt + self.res_drop1(tmp)
        tgt = self.ln1(tgt)
        ...
        ############################################################################
        # (1) the cross-attention block
        tmp = self.cross_attn(query=tgt, key=memory, value=memory, attn_mask=None)
        tgt = tgt + self.res_drop2(tmp)
        tgt = self.ln2(tgt)
        # (2) the feedforward block
        tmp = self.fc2(self.ffn_drop(self.act(self.fc1(tgt))))
        tgt = tgt + self.res_drop3(tmp)
        tgt = self.ln3(tgt)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        ############################################################################
        attn_out = self.self_attn(query=src,key=src,value=src,attn_mask=src_mask)
