...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # project image features to wordvec_dim and add sequence dimension
        # (N, D) -> (N, W) -> (N, 1, W)
        projected_features = self.visual_projection(features).unsqueeze(1)
    
        # create causal (future-masking) attention mask
        # (T, T)
        ...
        decoder_out = self.transformer(tgt=caption_embeddings,memory=projected_features, tgt_mask=tgt_mask)
